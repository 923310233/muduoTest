//
// Created by wushuohan on 19-5-23.
//
...
FCGI_BeginRequestBody makeBeginRequestBody(int role, int keepConnection) {
    ...
    /* 大于0长连接，否则短连接 */
    body.flags = static_cast<unsigned char> ((keepConnection) ? FCGI_KEEP_CONN : 0);
...
int makeNameValueBody(char *name, int nameLen,
    ...
    /* 如果 nameLen 小于128字节 */
    if (nameLen < 128) {
        *bodyBuffPtr++ = static_cast<unsigned char> (nameLen); //nameLen用1个字节保存
    } else {
        /* nameLen 用 4 个字节保存 */
        *bodyBuffPtr++ = static_cast<unsigned char> ((nameLen >> 24) | 0x80);
        *bodyBuffPtr++ = static_cast<unsigned char> (nameLen >> 16);
        *bodyBuffPtr++ = static_cast<unsigned char> (nameLen >> 8);
        *bodyBuffPtr++ = static_cast<unsigned char> (nameLen);
    }

    /* valueLen 小于 128 就用一个字节保存 */
    if (valueLen < 128) {
        *bodyBuffPtr++ = static_cast<unsigned char> (valueLen);
    } else {
        /* valueLen 用 4 个字节保存 */
        *bodyBuffPtr++ = static_cast<unsigned char> ((valueLen >> 24) | 0x80);
        *bodyBuffPtr++ = static_cast<unsigned char> (valueLen >> 16);
        *bodyBuffPtr++ = static_cast<unsigned char> (valueLen >> 8);
        *bodyBuffPtr++ = static_cast<unsigned char> (valueLen);
    }
    ...
    /* 计算出 body 的长度 */
    *bodyLen = static_cast<int >(bodyBuffPtr - startBodyBuffPtr);
    return 1;
}
...
void startConnect(FastCgi_t *c) {
    ...
    rc = connect(sockfd, reinterpret_cast<struct sockaddr *>(&server_address), sizeof(server_address));
    assert(rc >= 0);
...
int sendStartRequestRecord(FastCgi_t *c) {
    ...
    rc = write(c->sockfd_, reinterpret_cast<char *>(&beginRecord), sizeof(beginRecord));
...
int sendParams(FastCgi_t *c, const char *name, const char *value) {
    ...
    makeNameValueBody(const_cast<char *>(name), static_cast<int >(strlen(name)), const_cast<char *>(value),
                      static_cast<int >(strlen(value)), bodyBuff,
                      &bodyLen);
    ...
    memcpy(nameValueRecord, reinterpret_cast<char *>(&nameValueHeader), FCGI_HEADER_LEN);
...
int sendEndRequestRecord(FastCgi_t *c) {
    ...
    rc = write(c->sockfd_, reinterpret_cast<char *>(&endHeader), FCGI_HEADER_LEN);
    assert(rc == FCGI_HEADER_LEN);

    return 1;
}

char *readFromPhp(FastCgi_t *c, char *content) {
    FCGI_Header responderHeader;
    content = new char[CONTENT_BUFF_LEN];
//    char content[CONTENT_BUFF_LEN];
    ...
    /* 先将头部 8 个字节读出来 */
    char a[100000];
    ssize_t read1 = read(c->sockfd_, a, 1000);
    LOG_INFO << "读出了" << read1;
